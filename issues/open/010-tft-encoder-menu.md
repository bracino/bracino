...
Bench 2026-08-25: glyphs were Y-mirrored (ST7735 MY vs char RAM order); 2 Hz dark bar was full-row `fill_rect` at 500 ms; Home had only one cursor item so rotation looked dead. Glyphs sent bottom-first; live lines are cached (no row wipe); Home shows `enc N`; decoder is debounced A-edge. Serial `enc` prints A/B/SW. Gamma tables added. Human: LED brighter with **R3 = 100 Ω** (was 220). Encoder **works but bouncy** — trying another module before more firmware debounce.
