...
**Known open: intermittent task-WDT starvations.** Root-caused to **interrupt-level load**, not tasks: the panic-mode backtrace (bench-only `CONFIG_ESP_TASK_WDT_PANIC`, now armed) showed the blamed ui task *asleep* in `vTaskDelay` while IDLE starved — the load was a GPIO edge storm whose per-edge framework dispatch outran the in-handler rate limit. Fixes in: encoder A/B caps (v0.09), raw framework-free ISR + IO-MUX glitch filters, per-task TWDT subscription (a mis-subscription in app_main had also produced dump spam that wrecked scan timing — binds took 11 passes instead of 1). One isolated WDT event remains unexplained pending the panic trial. **The panic setting is bench-only — must be removed before the field image.**
