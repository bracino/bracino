...
**015 logger chain field-ready (2026-09-06):** the DN004 backend chain is verified end-to-end on the t520 backend (192.168.1.215) — auth'd mosquitto → commit-service → fsync'd JSONL at `/var/lib/bracino/commit/telemetry.jsonl`, BATCH_ACK gated on the commit watermark, LWT + `bracino/gateway/health` flowing. **Gateway ↔ node ESP-NOW bind proven on real firmware for the first time** (health=0, HEARTBEATs, batches committed 46/46 on first contact). GW fixes shipped during bring-up: `s` now reports fw hash + build id (gen_build_id port from 012), LED error blinks slowed to 200 ms (countable 2.0 s patterns), `raw/drop/last_rssi` rx counters (recv-cb truth, "deaf" vs "rejecting" split), ambient ADC channel corrected (GPIO33 = ADC1_CH5, was silently sampling floating GPIO35), `RANGE` plausibility guard, `ext_amb` + tap mV in `s`. **Issue [018](../issues/closed/018-bench-c3-tx-dead.md)**: a bench C3 transmitted zero radiated power (driver `tx_ok` while two receivers heard nothing) — root cause was depaneling damage nicking antenna + mid-layer trace. Commissioning RF smoke-test (pinned `hel` burst vs golden board, instrumented RX, ≤30 cm) is now **mandatory before any new node's protocol bring-up**. GW deployed to the boiler room 2026-09-06; wall-node reflash (`bdf4a44` + `min_tpo_tpu_delta_c` 1.5) follows once install-day history drains. JSONL volume ≈ 8,600 lines/day ≈ 1.7 MB/day (node 15 s + gw_ambient 30 s; gzips ~10× if the stage-C rotation compresses).
...
| `gateway` firmware | **Built + deployed 2026-09-06** (015): DN003/DN004 chassis, BATCH_ACK on commit watermark, LWT, status SoftAP + GPIO2 LED, `s` with fw id / raw rx counters / ambient. ESP-NOW bind to the wall node proven end-to-end |
