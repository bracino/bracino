...
**Field-build fixes (2026-09-03):** pump counters were NVS-persisted but only saved on pump **stop** — the bench pump runs continuously, so the lifetime total never got checkpointed and every reflash read as "re-zero". Now: save every **5 min of pump runtime** + on stop/clear/soft-reboot (max loss ≈ checkpoint interval). UI Diagnostics soft-reboot now flushes counters too (was serial-`reboot`-only). **Clear counts is two-click confirmed** (mirrors Reboot). Control Prog: `ct_confirm_s` was mislabeled as a second "dT min" — now **`ct wait`** (inert, CT not fitted, kept for registry stability). **Home shows UTC clock** `04 Sep 2026 17:57` (row 4; Pump+Set share row 3) — time comes from the DN003 TIME_SYNC anchor, so it appears once comms has anchored and survives `comms off`; after a reboot with no gateway it shows a placeholder until the next anchor. No timezone on node: UTC by design.
