...
**Field-build fixes (2026-09-03):** pump counters were NVS-persisted but only saved on pump **stop** — the bench pump runs continuously, so the lifetime total never got checkpointed and every reflash read as "re-zero". Now: save every **5 min of pump runtime** + on stop/clear/soft-reboot (max loss ≈ checkpoint interval). UI Diagnostics soft-reboot now flushes counters too (was serial-`reboot`-only). **Clear counts is two-click confirmed** (mirrors Reboot). **Control Prog: `ct_confirm_s` removed from the registry** (human decision 2026-09-03 — the CT is defunct in this design; reverses the earlier "kept for registry stability" call). Wire id 5 is permanently reserved (no renumber, nothing deployed ever saw it); `BBU_CONFIG_VER` 3. Params NVS blob bumps to v2 → **params reset to defaults once** on next boot (counters/boot mode survive). Its dead warn path now uses a fixed internal 10 s window. Mislabel fix superseded: there is no second "dT min" row at all anymore. **Home shows UTC clock** `04 Sep 2026 17:57` (row 4; Pump+Set share row 3) — time comes from the DN003 TIME_SYNC anchor, so it appears once comms has anchored and survives `comms off`; after a reboot with no gateway it shows a placeholder until the next anchor. No timezone on node: UTC by design.

**Known open: intermittent task-WDT — both causes fixed in tree, soak clean, awaiting field image.** (1) Encoder GPIO ISR storm: 2026-09-03 first dump, `ui` at `jal enc_take_steps`; **A/B now polled**. (2) Poll-fix soak still panicked while menu-surfing: `pdMS_TO_TICKS(5)==0` at HZ=100 — `vTaskDelay(0)` yields but does not block. **Fix: `vTaskDelay(1)` (10 ms).** Bench re-soak on `5697cef` (2026-09-03, ~10 min menu abuse + soft reboots): **zero TWDT events**. Remaining field-image step: remove `CONFIG_ESP_TASK_WDT_PANIC` from sdkconfig.defaults after bench-walking the counters/UI fixes, rebuild, flash.
...
| `node-bbu` firmware | **Loop in tree** — DESIGN_NOTE_002. User modes **Auto / Manual / Test / Off**. TFT+encoder UI **on protoboard, menus OK** (2026-08-28; 010 closed): Modern DOS 8×16, row DMA + done-wait, BGR palette. **012 field image complete & bench-walked (2026-09-02):** boots last saved mode (NVS, human-path writes only), persistent counters, soft reboot, DN003 param-table editor, comms diagnostics, ring 7680 w/ OOM degrade, capture-while-disabled, scan back-off. Encoder A/B now **polled** (no GPIO ISR) after panic-trial dump 2026-09-03; re-soak before field flash |
