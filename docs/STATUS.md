...
**Known open: intermittent task-WDT starvations — encoder ISR, fix in tree pending re-soak.** Root-caused to **interrupt-level load**, not tasks. Panic-mode dump 2026-09-03 (encoder-provoked, caps on, raw ISR + 1 ms cap + IO-MUX filter all in): IDLE starved, only IDLE hungry, `ui` at `jal enc_take_steps` (loop top — innocent). A 1 ms software cap does not help: dropped edges still pay ISR entry. **Fix: poll A/B on the 5 ms timer (no GPIO ISR).** Re-flash and re-run the encoder soak; **then remove `CONFIG_ESP_TASK_WDT_PANIC`** before the field image. Earlier 2026-09-02 dump showed `ui` asleep in `vTaskDelay` — same hog. Per-task TWDT subscription stays (a mis-subscription in app_main had produced dump spam that wrecked scan timing).
