...
**Known open: intermittent task-WDT — two causes, both in tree, pending re-soak.** (1) Encoder GPIO ISR storm: 2026-09-03 first dump, `ui` at `jal enc_take_steps`; **A/B now polled**. (2) Poll-fix soak still panicked while menu-surfing: `ui` had just returned from `enc_take_hold`, still innocent. `CONFIG_FREERTOS_HZ=100` makes `pdMS_TO_TICKS(5)==0` — `vTaskDelay(0)` yields but does not block, so ui stays Ready and IDLE never runs except during TFT DMA waits. **Fix: `vTaskDelay(1)` (10 ms).** Then strip `CONFIG_ESP_TASK_WDT_PANIC` for the field image.
