...
TFT + encoder (breadboard, issue 010): ST7735S on hardware SPI2 + DMA (GPIO9 SCK / GPIO4 SDA / GPIO3 DC / GPIO2 RESET, CS tied GND). Landscape 160×128, 8-row grid. Encoder A/B/SW = GPIO0 / GPIO1 / GPIO5; all three are polled on a 5 ms timer (gray-code table for A/B — no GPIO ISR; C3 has no PCNT). Click enters; hold ~0.8 s returns Home. Serial `enc` prints A/B/SW and `skip=` (invalid gray samples). Human (2026-08-26): image runs, encoder much better; doubled 6×8 font is rejected — next is a real 12×16 bitmap.
